...
void encrypt(char *pubfilename, char *infilename, char *outfilename)
{
    // read public key
    FILE *pubfile = fopen(pubfilename, "r");
    check_error(pubfile == NULL, "failed to open pubfile");

    RSA *rsa = PEM_read_RSAPublicKey(pubfile, NULL, NULL, NULL);
    check_error_openssl(rsa == NULL, "failed to read pubfile");

    fclose(pubfile);

    // read input, encrypt, write encrypted
    FILE *infile = fopen(infilename, "r");
    check_error(infile == NULL, "failed to open input file");
    ...
    while (!feof(infile))
    {
        size_t size = fread(data, sizeof(char), max_size, infile);
        check_error(ferror(infile), "failed to read infile");

        printf("read %ld\n", size);

        int result = RSA_public_encrypt(size, data, encrypted, rsa, padding_type);
        check_error_openssl(result == -1, "failed to encrypt");

        printf("write %d\n", result);

        fwrite(encrypted, sizeof(char), result, outfile);
        check_error(ferror(outfile), "failed to write outfile");

        print(data, size, encrypted, result);
    }
...
int main(int argc, char **argv)
{
    if (argc < 3)
    {
        fprintf(stderr, "Usage: %s <public key file> <input file> <output file>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    encrypt(pubkeyfile, inputfile, outputfile);
